...
def main():
    ...
    now = datetime.now()
    s3_excel_path = f'thunders/daily_report/{now.strftime("%Y/%m/%d/%H/%M/%S")}/daily_report.xlsx'
    s3_parquet_path = f'thunders/daily_report/{now.strftime("%Y/%m/%d/%H/%M/%S")}/daily_report.parquet'
    ...
    response = requests.get(
        url=f'{ENDPOINT_URL}/gw/operations/api/operationsExport/Daily',
        headers={
            'Accept': 'application/octet-stream',  # Indica que a resposta esperada é um arquivo binário
    ...
    if response.status_code == 200:
        ...
        try:
            ...
            s3.put_object(Bucket=BUCKET_NAME, Key=s3_parquet_path, Body=parquet_buffer.getvalue())
