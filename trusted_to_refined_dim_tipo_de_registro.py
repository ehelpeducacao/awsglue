...
prefix = 'sales_force/tipos_de_registros/'
...
df = df_trusted.select(
    f.trim(f.col("id")).alias("idTipodeRegistro"),
    f.col("name").alias("nomeTipodeRegistro"),
    f.coalesce(f.col("description"), f.lit("N/A")).alias("descricao"),
    f.coalesce(f.col("sobjecttype"), f.lit("N/A")).alias("tipodeObjeto"),
...
output_path = "s3a://refined-zone-echo/dim_tipo_de_registro/"
df.write \
    .mode("overwrite") \
